...
const handleSubmit = async (e) => {
  ...
  console.log("Submitting review with data:", {
    ...formData,
    token,
    profilePicture: formData.profilePicture?.name || "No file selected",
  });
  try {
    const response = await fetch(
      "https://cervino-ceramix-backend-production.up.railway.app/admin/review/addReview",
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      ...
      const allReviewsResponse = await fetch(
        "https://cervino-ceramix-backend-production.up.railway.app/admin/review",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
